...
function toWeirdCase(s) {
  return s.split(' ').flatMap(str => str.split('').map((alph, i) => i % 2 ? alph.toLowerCase() : alph.toUpperCase()).join('')).join(' ');

  // let upperStr = '';

  // s.split(' ').map(str => {
  //   str.split('').map((alph, i) => {
  //     upperStr += i % 2 ? alph.toLowerCase() : alph.toUpperCase();
  //   });
  //   upperStr += ' ';
  // }).join('');
